...
const baseURL =
  "http://ec2-3-26-176-3.ap-southeast-2.compute.amazonaws.com:8080/mosqueSummary/mosqueSummaryDetails/June-2018";
  ...
  const onWheel = (event) => {
    ...
    let newXDomain = xDomain.map((date) => {
      const newDate = new Date(
        pointerDate.valueOf() +
          (date.valueOf() - pointerDate.valueOf()) * zoomFactor
      );
  ...
  return data === null ? (
    ...
    <div className="App" onWheel={onWheel}>
      <div className="svg-buttons-div">
        <button className="svg-buttons" onClick={() => moveTimeWindow(-1)}>
          Move Left
        </button>
        <button className="svg-buttons" onClick={() => moveTimeWindow(1)}>
          Move Right
      ...
      <svg width={width} height={height} className="svg-timeline">
        <Group top={margin.top}>
          ...
          {ticks(
            xDomain[0],
            xDomain[1],
            xDomain[1].getFullYear() - xDomain[0].getFullYear()
          ).map((d, i) => {
